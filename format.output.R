...
format.output <- function(country.estimation, one.sided.est.country, real.rate.country, start, end, run.se = TRUE) {
    ...
    output.country[,1]   <- seq(from = (as.Date(ti(shiftYear(start,-1),'annual'))+1), to = (as.Date(ti(shiftYear(end,-1),tif='annual'))+1), by = 'year')
    ...
    if (run.se) {
        output.country[,17]   <- seq(from = (as.Date(ti(shiftYear(start,-1),'annual'))+1), to = (as.Date(ti(shiftYear(end,-1),tif='annual'))+1), by = 'year')
